...
const GraphBody = ({ data }: { data: Course[] }) => {
  const chartData: ChartData<"bar">[] = data.map((course, index) => {
    ...
    if(course.A + course.B + course.C + course.D + course.F + course.NR > 0){
      ...
      dataset.data = [course.A, course.B, course.C, course.D, course.F, course.NR];
      ...
      tooltip: {
        ...
        callbacks: {
          label: (context) => {
            ...
            const total = dataset.reduce((accumulator, currentValue) => {return accumulator + currentValue}, 0);
        ...
        return (
          <div key={index} className="border-1 mx-3 p-3">
            ...
            <div className="pl-15">
              ...
              <p>Withdraws: {W}</p>
              {(A + B + C + D + F) ? (
                <p title="Pass rate includes letter grades from A-D">Pass Rate: {((A + B + C + D) / (grade_regs - W) * 100).toFixed(2)}%</p>
              ):
              (
                <p title="S is passing, U is not passing">Pass Rate: {((S) / (grade_regs - W) * 100).toFixed(2)}%</p>
              )
              }
            </div>
          </div>
